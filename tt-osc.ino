...
Adafruit_PCD8544 lcd = Adafruit_PCD8544(2, 3, 4, 6, 7); // пины к которым у вас подключен дисплей

//#define KEY_L  13 // кнопка ЛЕВО  (можно любой пин)
#define KEY_OK A3 // кнопка ОК    (можно любой пин)
#define KEY_R  A4 // кнопка ПРАВО (можно любой пин)
#define VBAT   A5 // любой свободный аналоговый пин для измерения напряжения АКБ 
#define GEN_PIN  10  // пин для генератора сигналов (не менять)
#define DDS_PIN  10 // пин для генератора dds (не менять)

#define KEY_DELAY 200

uint8_t mode = 0; // пункт главного меню
...
//    if (digitalRead(KEY_L)) { if (++mode==255) mode = modes - 1; delay(250);}
  ...
  if(mode<modes-1) {
    pinMode(A0, OUTPUT);
    digitalWrite(A0, LOW); // подключаем А0 к земле
  }

  switch (mode) {
    case 0:
//      FreqCount.begin(1000);
      Oscil(); // "выпадаем" в осцилл
      break;
    case 1:
      InitTimersSafe();
      SetPinFrequencySafe(GEN_PIN, freq);
      pwmWrite(GEN_PIN, PWM*2.55);
      Generator(); // "выпадаем" в генератор 
      break;
    case 2:
      InitTimersSafe();
      SetPinFrequencySafe(DDS_PIN, 200000);
      DdsGenerator(); // "выпадаем" в DDS генератор
      break;
//    case 3: Terminal(); break;
//    case 4: OscilSlow(); break;
    case 3:
//      setupTT();
      break;
    default:
...
void ShowModeInv(char y) { // выводим режим работы. m - режим, y - строка, где рисовать
  ...
  lcd.setCursor(3, y);  
  switch (mode) { // lcd.print(&modeStr[mode]); - так почему то не работает....
...
//    case 3: lcd.print("UART-терминал"); break;
    case 3: lcd.print("   Т-Тестер  "); break;
...
void DdsGenerator() {
  ...
  lcd.clearDisplay(); ShowModeInv(0); // режим работы, заголовок

while(1) {
  for (uint8_t i=0; i<84;) lcd.drawLine(i, 36-pgm_read_byte(&ddsWave[menu][i%32])/10, i, 36-pgm_read_byte(&ddsWave[menu][(i++)%32])/10, BLACK);
  lcd.setCursor(3, 40);  lcd.print(ddsStr[menu]);  lcd.display(); // отрисовали все  
...
void Terminal() {
  ...
  lcd.clearDisplay(); 
  ShowModeInv(0);  
  lcd.setCursor(15,15); lcd.println("Скорость:");
  lcd.setTextColor(WHITE, BLACK); 
  lcd.setCursor(66, 25); lcd.println("+");
  lcd.setCursor(13, 25); lcd.print("-");
  lcd.setTextColor(BLACK);
  lcd.println(" ");      
  lcd.print(speedUart[menu]);
  lcd.println(" ");
  lcd.setCursor(9,40); lcd.println("ОК - запуск");
...
uint32_t frequency(){
  ...
  delay(100);// Время измерения
  TCCR1B=0;
  return 10*(((uint32_t)int_tic<<16) | TCNT1); //сложить что натикало
}
