/*                      \\\|///
  ...
  /--------------------oOOo-(_)-oOOo---------------------\
  |                                                      |
  |           Oscilloscope, DDS Generator and            |
...
Adafr_PCD8544 lcd = Adafr_PCD8544(2, 3, 4, 7); // пины к которым подключен дисплей

//#define KEY_L  13 // кнопка ЛЕВО  (можно любой пин)
#define KEY_OK A4 // кнопка ОК    (можно любой пин)
#define KEY_R  A5 // кнопка ПРАВО (можно любой пин)

#define KEY_DELAY 200

const char modeStr0[] PROGMEM = " Осциллоскоп ";
const char modeStr1[] PROGMEM = "PWM-генератор";
const char modeStr2[] PROGMEM = "DDS-генератор";
const char modeStr3[] PROGMEM = "UART-терминал";
const char modeStr4[] PROGMEM = "   Т-Тестер  ";
//const char modeStr5[] PROGMEM = "Лог.анализатор";

void ShowModeInv(char y, uint8_t m) { // выводим режим работы. m - режим, y - строка, где рисовать
//  lcd.drawLine(0, y, 83, y, BLACK);
  lcd.setTextColor(WHITE, BLACK);
//  lcd.setCursor(0, y+1); lcd.println(" "); lcd.setCursor(78, y+1); lcd.println(" ");  // для красоты только
  lcd.setCursor(3, y);
//  lcd.print(modeStr[m]); // выводим режим работы
  switch (m) {
    case 0: lcd_pgm_string(modeStr0); break;
    case 1: lcd_pgm_string(modeStr1); break;
    case 2: lcd_pgm_string(modeStr2); break;
    case 3: lcd_pgm_string(modeStr3); break;
    case 4: lcd_pgm_string(modeStr4); break;
//    case 0: lcd.print(" Осциллоскоп "); break;
//    case 1: lcd.print("  Генератор  "); break;
//    case 2: lcd.print("DDS-генератор"); break;
//    case 3: lcd.print("UART-терминал"); break;
//    case 4: lcd.print("   Т-Тестер  "); break;
//    case 5: lcd.print(" Снятие АЧХ  "); break;
  }
  lcd.setTextColor(BLACK);
}

void setup()  {
  uint8_t VBAT = A3; // аналоговый пин для измерения напряжения АКБ
  float VCC = ReadVcc(); // напряжение питания
  float d = analogRead(VBAT)*VCC/1024; // измеряемое напряжение батареи
  uint8_t option = 0; // пункт главного меню
...
//  pinMode(KEY_L, INPUT);
  pinMode(KEY_R, INPUT); // настраиваем кнопки на вход
  lcd.begin();
//  lcd.setContrast(contrast);
  ...
  while(digitalRead(KEY_OK)) {  // цикл, пока не нажали кнопку ОК    
    ...
    if (!digitalRead(KEY_R)) {
      if (++option > modes - 1) option = 0;
      d = analogRead(VBAT)*VCC/1024; // измеряем напряжение батареи
      delay(250);
      }
//    if (digitalRead(KEY_L)) { if (++mode==255) mode = modes - 1; delay(250);}

    lcd.clearDisplay();  // чистим буфер экранчика
    lcd.setCursor(2, 2); lcd.setTextColor(WHITE, BLACK); lcd.print(d); lcd.print("V / "); lcd.print(VCC); lcd.print("V"); lcd.setTextColor(BLACK);
    ShowModeInv(29, option);
    lcd.display();       
  } // цикл нажатия ОК  
  // нажали кнопку ОК из меню, инициализируем и выходим из меню

  // антидребезг кнопки ОК
//  for (char a=29; a>0; a-=2) {
//    lcd.clearDisplay();
//    ShowModeInv(a, option);
//    lcd.display();
//    }
  delay(200);

  switch (option) { // Прыгаем в выбранный режим из главного меню
    case 0 : Oscil();        break; // осцилл
    case 1 : Generator();    break; // генератор 
    case 2 : DdsGenerator(); break; // DDS генератор
    case 3 : Terminal();     break; // USART приемник
    case 4 : Ttester();    break; // тестер
  }  
}
...
// ==== Считывание напряжения питания (Vcc) ====
...
void ReadAdc(uint8_t adcBuf[], uint16_t BUFSIZE, uint8_t razv) {
  if (razv%10) { // (razv>0)        // если развертка без задержек всяких
    ...
    for(uint16_t i=0; i<BUFSIZE; i++) {  // цикл для чтения  
...
void Oscil() {
  uint32_t count = 0;
  uint16_t countX = 0;
  uint8_t syncLevel = 30; // уровень синхронизации 
  bool vRef = 1;          // флаг опорного напряжения
  bool pause = 0;         // флаг режима паузы
  uint8_t razv = 0;  
  uint16_t pOffset = 0;   // смещение графика в режиме паузы
  uint16_t grOffset = 0;  // смещение графика в рабочем режиме
  float VCC = 5.0;        // напряжение питания, меряем мультиметром
  uint16_t Vmax = 0;      // максимальное напряжение
  uint16_t BUFSIZE = 700;
  uint8_t adcBuf[BUFSIZE + 1];
...
while(1) {
  if (!pause) ReadAdc(adcBuf, BUFSIZE, razv); // если нет паузы, то снимаем осциллограмму
    // обрабатываем кнопки
//  if (digitalRead(KEY_L))   switch (menu) {
//    case 0 : vRef=!vRef;                               break;
//    case 1 : razv--; if(razv==255) razv=0;             break;
//    case 2 : pOffset-=10; if (pOffset<0) pOffset=0;    break;
//    case 3 : syncLevel-=20; if (syncLevel<20) syncLevel=20;        break;
//  }
  if (!digitalRead(KEY_R))  switch (menu) {
    case 0 : vRef=!vRef;                               break;
    case 1 : razv++; if (razv==7) razv=0;              break;
    case 2 : pOffset+=10; if(pOffset>620) pOffset=620; break;
    case 3 : syncLevel+=20; if(syncLevel>230) syncLevel=20;       break;
  }       
  if (!digitalRead(KEY_OK)) { if (++menu==4) {menu=0; pause=0; } } // перебор меню
  
  // Определение точки синхронизации 
  uint8_t flagSINHRO=0;
  uint8_t flagSINHRnull=0;
  for (uint8_t y=1; y<255; y++) {
    if (!flagSINHRO && adcBuf[y]<syncLevel) flagSINHRnull=1;
    if (!flagSINHRO && flagSINHRnull && adcBuf[y]>syncLevel) { flagSINHRO=1; grOffset=y; }
  }
  // максимальное значение сигнала
  Vmax=0; 
  for (uint8_t y=1; y<255; y++) if (Vmax<adcBuf[y]) Vmax=adcBuf[y];
  
  // отрисовка графика
  lcd.clearDisplay();
  if (Vmax==0xFF) for(int y=0; y<4; y++) lcd.drawLine(y,9,y,47, BLACK);
  for(uint8_t y=0; y<80; y++) {   
   lcd.drawLine(y+4, 47-adcBuf[y+(pause ? pOffset : grOffset)]/7, y+4, 47-adcBuf[y+(pause ? pOffset : grOffset)+1]/7,   BLACK); // НАДО БУДЕТ ОБЪЕДЕНИТЬ POFFSET И GROFFSET!
  }      
  if (pause) { // рисуем прокрутку в режиме паузы
    lcd.drawLine(pOffset/8,8,pOffset/8+6,8, BLACK); // шкала прокрутки
    lcd.drawLine(pOffset/8,9,pOffset/8+6,9, BLACK); // шкала прокрутки    
  } else {
    lcd.fillCircle(80,47-syncLevel/7, 2, BLACK); // рисуем уровень синхронизации    
    grOffset = 0;                               // сброс синхронизации
  }
  // === Отрисовка сетки ===
  for(uint8_t i=33;i>5;i=i-7) { lcd.drawLine( 0, i, 2, i, BLACK); } // черточки слева  
  for(uint8_t i=47;i>5;i=i-3) { lcd.drawPixel(21,i, BLACK);lcd.drawPixel(42,i, BLACK);lcd.drawPixel(63,i, BLACK); }
  for(uint8_t i=3;i<84;i=i+3) { lcd.drawPixel(i,33, BLACK);lcd.drawPixel(i,19, BLACK);}    
    
  // === Отрисовка меню осцилла ===
  lcd.setCursor(0,0);    
  if (menu==0) lcd.setTextColor(WHITE, BLACK); else lcd.setTextColor(BLACK);
  if (vRef) lcd.print(VCC,1); else lcd.print("1.1");
  if (menu==1) lcd.setTextColor(WHITE, BLACK); else lcd.setTextColor(BLACK);
  lcd.print(" ");  lcd.print(razv);
  if (menu==2) { lcd.setTextColor(WHITE, BLACK); pause=1; } else lcd.setTextColor(BLACK);
  lcd.print(" P");
  lcd.setTextColor( BLACK);
  if (menu==3) { pOffset=0; pause=0; 
    lcd.fillCircle(80, 47-syncLevel/7, 5, BLACK);  lcd.fillCircle(80, 47-syncLevel/7, 2, WHITE);}
...
//  if (FreqCount.available()) count = FreqCount.read(); //вывод частоты по готовности счетчика частоты сигнала
  ...
  uint8_t Frec1=0;
  uint32_t Frec=0;
  bool flagFrec1=0, flagFrec2=0, flagFrec3=0;
  for(uint8_t y=1; y<255; y++)  {
    if(!flagFrec1 && adcBuf[y]<syncLevel) flagFrec2=1;
    if(!flagFrec1 && flagFrec2 && adcBuf[y]>syncLevel) { flagFrec1=1; Frec1=y; } 
    if( flagFrec1 && adcBuf[y]<syncLevel) flagFrec3=1;
    if( flagFrec3 && adcBuf[y]>syncLevel) {
  ...
  //#######################################частоты сигнала
  if(vRef) {    
    if((Vmax*VCC/255)>2.5) countX=count*(OVERCLOCK/16.0); else countX=Frec*(OVERCLOCK/16.0);
  } else countX=Frec*(OVERCLOCK/16.0); // можно оптимизнуть......... пока лень.
  // выводим частоту    
  if (countX<1000) { lcd.print(" "); lcd.print(countX); lcd.print("Hz");}
             else  { float countXK=countX/1000.0; lcd.print(countXK,1); lcd.print("KHz");}
  lcd.setCursor(0,40); lcd.setTextColor(BLACK);
  if (vRef) lcd.print(Vmax*VCC/255,1);
         else  lcd.print(Vmax*1.1/255,1);
  lcd.print("V");  
  delay(200);  
  lcd.display();
}
...
// РЕЖИМ ГЕНЕРАТОРА
void Generator() {
  uint8_t GEN_PIN = 10;  // пин для генератора сигналов
  uint8_t PWM = 50;//стартовое значение ШИМ от 0 до 100 для генератора
  uint16_t freq = 500; // стартовое значение частоты в Гц для генератора
  uint16_t stepFreq = 0;
  uint8_t menu = 0;     // пункт меню

  InitTimersSafe();
  SetPinFrequencySafe(GEN_PIN, freq);
...
//    if(digitalRead(KEY_L)) { PWM--; if(PWM<0)   PWM=100; delay(KEY_DELAY); }
    if(!digitalRead(KEY_R)) { if(++PWM>100) PWM=0; delay(KEY_DELAY); }
    pwmWrite(GEN_PIN, PWM*2.55); // задали ШИМ
    lcd.setTextColor(WHITE, BLACK); // если меняем шим, то рисуем его инверсным
    lcd.drawLine(15, 39, 68+(PWM==100?6:0)+(PWM<10?-6:0), 39, BLACK); // сверху полоска для большей инверсности
  } else { // меняем частоту
...
//    }
    ...
    lcd.setTextColor(BLACK); // если ШИМ не меняем, то выбираем обычный текст
    #define XFREQ 14 // ======== // и, выделяем декаду частоты полосочками
    stepFreq=pow(10, (uint8_t)menu); if (menu>1) stepFreq++; // устраняем глючность pow, почему 10 в степени 2 = 99?
    uint8_t menu_t = menu; if (menu>2) menu_t++; if (menu==6) menu_t++; // делаем табуляцию частоты
    menu_t = 54 - menu_t * 6; // считаем положение полосочек
    lcd.drawLine(menu_t, XFREQ-2, menu_t+4, XFREQ-2, BLACK); // рисуем полоски
    lcd.drawLine(menu_t, XFREQ-3, menu_t+4, XFREQ-3, BLACK);
    lcd.drawLine(menu_t, XFREQ+8, menu_t+4, XFREQ+8, BLACK);          
    lcd.drawLine(menu_t, XFREQ+9, menu_t+4, XFREQ+9, BLACK);
  }
  // рисуем уровень ШИМ (инверсия текста задана ранее)
  lcd.setCursor(15, 40); // ставим курсор))
  lcd.print(" PWM=");  lcd.print(PWM);  lcd.print("% "); // выводим уровень ШИМ
  lcd.setTextColor(BLACK); // убираем инверсию при выводе частоты
  // рисуем частоту с табуляцией  
  lcd.setCursor(6, XFREQ);
  for (unsigned long freq_t = 1000000; freq_t>0; freq_t/=10) {
  if (freq>=freq_t) {
    lcd.print((freq/freq_t)%10); if (freq_t==1000000 || freq_t==1000) lcd.print("'"); }  else { 
    lcd.print("_");              if (freq_t==1000000 || freq_t==1000) lcd.print(" "); }
  }
  lcd.print(" Hz");  
  // отрисовка графика PWM
  for (char x=17; x<67; ) {
    if (PWM!=0)             lcd.drawLine(x, 26, x+PWM/4, 26, BLACK); x+=PWM/4;       // верх 
    if (PWM!=0 && PWM!=100) lcd.drawLine(x, 26, x, 36, BLACK);                       // спад
    if (PWM!=100)           lcd.drawLine(x, 36, x+25-PWM/4, 36, BLACK); x+=25-PWM/4; // низ
    if (PWM!=0 && PWM!=100 && x<43) lcd.drawLine( x, 36, x, 26, BLACK);              // подъем
  }
  lcd.display(); // вываливаем буфер на дисплей
  }
}

// === DDS генератор ===
const char ddsStr0[] PROGMEM = "    Синус";
const char ddsStr1[] PROGMEM = " Треугольник";
const char ddsStr2[] PROGMEM = "    Пила";
const char ddsStr3[] PROGMEM = "Обратная пила";

void DdsGenerator() {
  static const uint8_t ddsWave[][32] PROGMEM = {        
  ...
  uint8_t GEN_PIN = 10; // пин для генератора dds
  uint8_t ddsCount=0;
  uint8_t menu = 0;     // пункт меню
  
  InitTimersSafe();
  SetPinFrequencySafe(GEN_PIN, 200000);
  
  // Рисуем DDS-генератор
while(1) {
  lcd.clearDisplay(); ShowModeInv(0, 2); // режим работы, заголовок
  for (uint8_t i=0; i<84;) lcd.drawLine(i, 36-pgm_read_byte(&ddsWave[menu][i%32])/10, i, 36-pgm_read_byte(&ddsWave[menu][(i++)%32])/10, BLACK);
  lcd.setCursor(3, 40);
//  lcd.print(ddsStr[menu]);
  switch (menu) {
      case 0: lcd_pgm_string(ddsStr0); break;
      case 1: lcd_pgm_string(ddsStr1); break;
      case 2: lcd_pgm_string(ddsStr2); break;
      case 3: lcd_pgm_string(ddsStr3); break;
    }
  lcd.display(); // отрисовали все  
  while(digitalRead(KEY_R)) { // выводим выбранный сигнал, пока не нажали кнопку
    pwmWrite(GEN_PIN, pgm_read_byte(&ddsWave[menu][(ddsCount++)&0x1F]));
    //ResetAvrCheck();
  }
...
// UART приемник
const char termLine1[] PROGMEM = "Скорость <>";
const char termLine2[] PROGMEM = "ОК - запуск";

void Terminal() {
  const long speedUart[] = { 115200,57600,38400,19200,9600,4800,2400,1200 };
  uint8_t menu = 0;     // пункт меню
  ...
  ShowModeInv(0, 3);  
  lcd.setCursor(15,15);
  lcd_pgm_string(termLine1);
  lcd.setCursor(25, 25);
  lcd.print(speedUart[menu]);
  lcd.setCursor(9,40);
  lcd_pgm_string(termLine2);
//  if(digitalRead(KEY_L))  { if(--menu==255) menu=7;   delay(KEY_DELAY);  }
  if(!digitalRead(KEY_R))  { if(++menu==8)   menu=0;   delay(KEY_DELAY);  }
  if(!digitalRead(KEY_OK)) {
    ...
    while (1) {
      ...
      if (Serial.available()) { // Если в буфере есть данные
        ...
        lcd.print((char)(Serial.read())); // печать байта в дисплей
        lcd.display(); // выводим символ
      } 
    }         
  } 
  lcd.display();
...
uint32_t frequency(){
  ...
  TCCR1B=0;
  return ((uint32_t)int_tic<<16) | TCNT1; //сложить что натикало
}
...
const unsigned char Hello1[] PROGMEM = "Transistor";
const unsigned char Hello2[] PROGMEM = "Tester";
const unsigned char Hello3[] PROGMEM = "for Arduino";
const unsigned char Hello4[] PROGMEM = "v1.08.004";
...
//  startMenu();
  ...
  lcd.clearDisplay();
  lcd_set_cursor(0, 0);
  lcd_pgm_string(Hello1);
  lcd_set_cursor(1, 0);
  lcd_pgm_string(Hello2);
  lcd_set_cursor(2, 0);
  lcd_pgm_string(Hello3);
  lcd_set_cursor(3, 0);
  lcd_pgm_string(Hello4);
